...
parser = argparse.ArgumentParser(description="Run simulated offline-online RL with configurable state abstraction.")
...
parser.add_argument('--k', type=int, default=0, help="Number of clusters (used if state_abstraction is k_means or k_means_features)")
...
if state_abstraction == 'False':
    ...
    k = None
elif state_abstraction == 'last_action' or state_abstraction== 'structural':
    k = None

else:
    #for 'k_means' or 'k_means_features', use provided k
    k = int(args.k)

dataset = 'simulation'
num_times = 5 #for offline training (number of times we go through the df)
num_episodes_online = 30000 #for online training
...
if state_abstraction == 'k_means':
    ...
    save_path = os.path.join(DATA_FOLDER, f'df_simulation_{k}_clusters.csv')
    df.to_csv(save_path, index=False)

elif state_abstraction == 'k_means_features':
    clustering_state_cols = ['loan_amount','credit_score','debt_to_income','purpose']
    df, kmeanModel = k_means(df, clustering_state_cols, k)
    save_path = os.path.join(DATA_FOLDER, f'df_simulation_{k}_clusters_features.csv')
    df.to_csv(save_path, index=False)
    ...
    save_path = os.path.join(DATA_FOLDER, f'df_{dataset}_{k}_structural.csv')
    df_structural.to_csv(save_path, index=False)
...
training_filename = os.path.join(results_dir, f"{dataset}_{state_abstraction}_{benchmark}_training_{k}.pkl")
testing_filename = os.path.join(results_dir, f"{dataset}_{state_abstraction}_{benchmark}_testing_{k}.pkl")
...
online_filename = os.path.join(results_dir, f"{dataset}_{state_abstraction}_{benchmark}_online_{k}.pkl")

with open(online_filename, 'wb') as f:
    pickle.dump(training_results, f)
