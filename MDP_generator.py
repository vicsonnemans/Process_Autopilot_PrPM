...
class InterpretableLoanMDP(gym.Env):
    def __init__(self, seed=42):
        ...
        #allowable transitions by action index
    ...
    def _generate_random_applicant(self):
        features = []
        for name, (low, high) in self.feature_config.items():
            if name == "loan_amount":  
    ...
    def reset(self):
        ...
        self.creditworthy = int(np.dot(self.credit_weights, self.static_state) > self.approval_threshold)
...
filename = 'df_simulation_preprocessedv2.csv'
...
if not os.path.exists(filename): #generate static event log
    ...
    df.to_csv(f'df_simulation_preprocessedv2.csv', index=False)
    df.head(10)